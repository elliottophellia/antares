...
func TestSkillTogglePersistsWithoutMutatingReadOnlySourceAndRestarts(t *testing.T) {
	...
	restartedManager := skills.NewManager(persisted.Skills.Dirs)
...
func TestSkillHandlersObserveManagerReplacedDuringReload(t *testing.T) {
	home := t.TempDir()
	...
	writeServerSkill(t, oldDir, "replace-me")
	writeServerSkill(t, oldDir, "old-only")
	writeServerSkill(t, newDir, "replace-me")
	writeServerSkill(t, newDir, "replacement-only")
	writeServerSkill(t, packDir, "code-review")
	...
	oldManager := skills.NewManager([]string{oldDir})
	if err := oldManager.Reload(); err != nil {
		t.Fatal(err)
	}
	a := &agent.Agent{}
	a.SetConfig(cfg)
	a.SetSkills(oldManager)
	var replacement *skills.Manager
	s := New(Options{
		Config: cfg,
		Agent:  a,
		Skills: oldManager,
		Reload: func() error {
			...
			replacement = skills.NewManager([]string{newDir, packDir})
			replacement.SetPackDirs([]string{packDir})
	...
	if replacement == nil || s.currentSkills() != replacement {
		t.Fatal("reload replacement is not the authoritative manager")
	}
	if s.skills != oldManager {
		t.Fatal("applyReload copied the agent manager into the fallback field")
	}
	if s.commandDeps().Skills != replacement {
		t.Fatal("command dependencies retained the stale skill manager")
	}
	if sk, ok := replacement.Get("replace-me"); !ok || sk.Enabled {
		t.Fatalf("replacement manager preference = %#v, found=%v; want disabled", sk, ok)
	}
...
func newSkillToggleServer(t *testing.T, names, disabled []string) (*Server, *skills.Manager, string, map[string]string) {
	...
	for _, name := range names {
		sources[name] = writeServerSkill(t, dir, name)
	}
	cfg := config.Default()
	cfg.Skills.Enabled = true
	...
	manager := skills.NewManager([]string{dir})
...
func writeServerSkill(t *testing.T, dir, name string) string {
	t.Helper()
